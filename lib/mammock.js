...
Server.prototype.start = function () {
    ...
    this._internals.server.on("request", function (request, response) {
        ...
        if (!blacklisted && !missing && !invalidContent) {
            ...
            //var module = require(_this.options.root + "/" + route);
            var endpoint = new (require(_this.options.root + "/" + route))();

            if (request.method === "GET" && endpoint.get && typeof endpoint.get === "function") {
                _this.info("Serving GET request...");
                var getResponse = endpoint.get(route, request, response);
                _this.info("Writing header response to GET request...");
                response.writeHeader(getResponse.status, {
                    "Content-Type": "application/json"
                });
                _this.info("Writing response to GET request...");
